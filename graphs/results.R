...
for (i in dflist) {
  ...
  tree.cs.temp <- read.table(paste("../results/fat",i,"cs-trace.txt",sep=""), header=T)
  tree.app.temp <- read.table(paste("../results/fat",i,"app-trace.txt",sep=""), header=T)
  tree.cs.temp$scen=i
  tree.app.temp$scen=i
  tree.cs <- rbind(tree.cs,tree.cs.temp)
  tree.app <- rbind(tree.app,tree.app.temp)
  
  lin.cs.temp <- read.table(paste("../results/ar",i,"cs-trace.txt",sep=""), header=T)
  lin.app.temp <- read.table(paste("../results/ar",i,"app-trace.txt",sep=""), header=T)
  lin.cs.temp$scen=i
  lin.app.temp$scen=i
...
g.hits.lin <- ggplot(lin.cs.hits.sum, aes(x=ordered(scen), y=tot/80, fill=factor(Node))) + geom_bar(stat="identity") + 
  ylab("Cache Hit Ratio [%]") + xlab("Cache Admission Rate [%]")
...
g.hopcount <- ggplot(tree.app.mean, aes(x = factor(scen), y=HopCount)) + 
  geom_boxplot()
