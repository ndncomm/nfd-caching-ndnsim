...
for (i in dflist) {
  ...
  tree.cs.temp <- read.table(paste("../results/fat_lcd",i,"cs-trace.txt",sep=""), header=T)
  tree.app.temp <- read.table(paste("../results/fat_lcd",i,"app-trace.txt",sep=""), header=T)
  tree.cs.temp$scen=i
  tree.app.temp$scen=i
  tree.cs <- rbind(tree.cs,tree.cs.temp)
  tree.app <- rbind(tree.app,tree.app.temp)
  
  lin.cs.temp <- read.table(paste("../results/lin_lcd",i,"cs-trace.txt",sep=""), header=T)
  lin.app.temp <- read.table(paste("../results/lin_lcd",i,"app-trace.txt",sep=""), header=T)
  lin.cs.temp$scen=i
  lin.app.temp$scen=i
